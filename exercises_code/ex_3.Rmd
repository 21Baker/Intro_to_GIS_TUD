...
nl_15 <- nc_open(here("data","raw","NASAdata.nc"))
...
night <- raster((light.slice), xmn=min(lon), xmx=max(lon), ymn=min(lat), ymx=max(lat), crs=CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs+ towgs84=0,0,0"))
...
ita_mask <- rast(ita_mask)
ita_pop_mask <- rast(ita_pop_mask)
...
ita_pop_mask -> raster(ita_pop_mask)
